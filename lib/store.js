...
class ModelBase extends EventEmitter {
  ...
  static subscribe(actionWrapper, handler) {
    ...
    if (! (actionType in this.staticMapping)) this.staticMapping[actionType] = new Set();

    this.staticMapping[actionType].add(handler);
  }
...
const createModel = (schema = {}, dispatcher = Dispatcher.main()) => {
  ...
  dispatcher.register((data) => Model.dispatch(data));
  return Model;
};
