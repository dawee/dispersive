...
class Action {
  ...
  buildWrapper() {
    ...
    wrapper.with = (...prefill) => (
      (...argv) => this.callHandler(...(prefill.concat(argv)))
    );
  ...
  callHandler(...argv) {
    const result = (this.handler || noop)(...(this.prefill.concat(argv)));
    const promise = (result instanceof Promise) ? result : new Promise(
