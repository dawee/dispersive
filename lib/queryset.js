...
class QuerySet extends EventEmitter {
  ...
  _filterFromExpression(expression) {
  ...
  _filterFromPredicate(predicate) {
    return this.createQuerySet({predicate});
  }

  filter(expressionOrPredicate) {
    if (typeof expressionOrPredicate === 'function') {
      return this._filterFromPredicate(expressionOrPredicate);
    }

    return this._filterFromExpression(expressionOrPredicate);
  }

  _excludeFromExpression(expression) {
    this.assertExpressionInSchema(expression);
  ...
  _excludeFromPredicate(predicate) {
    return this.createQuerySet({
  ...
  exclude(expressionOrPredicate) {
    if (typeof expressionOrPredicate === 'function') {
      return this._excludeFromPredicate(expressionOrPredicate);
    }

    return this._excludeFromExpression(expressionOrPredicate);
  }
